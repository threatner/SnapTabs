...
- **Rating prompt**: `shouldShowRatingPrompt` = not dismissed and (≥ `RATING_PROMPT_AFTER_MANUAL_SNAPSHOTS` (5) manual snapshots or ≥ `RATING_PROMPT_AFTER_RESTORES` (3) restores). The popup shows `RatingPrompt.svelte` once; rating or dismissing sets `ratingPromptDone`. Skipped on Edge.
